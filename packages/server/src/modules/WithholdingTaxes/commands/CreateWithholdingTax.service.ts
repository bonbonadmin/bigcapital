...
import { CreateWithholdingTaxDto } from '../dtos/WithholdingTax.dto';
...
@Injectable()
export class CreateWithholdingTaxService {
  constructor(
    @Inject(Account.name)
  ...
  public async createWithholdingTax(withholdingTaxDTO: CreateWithholdingTaxDto) {
    await this.validateAccount(withholdingTaxDTO.accountId);

    return this.withholdingTaxModel().query().insert(withholdingTaxDTO);
  }
}
